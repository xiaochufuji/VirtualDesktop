...
#pragma region common function defination
	...
	{
#ifndef UNICODE
		std::string _guid = "guid: ";
		std::string _name = "\tname: ";
		std::string _wallpaper = "\twallpaper: ";
		std::string _index = "\tindex: ";
		os << _guid << PrintGUID(info.guid)
			<< _name << info.desktopName
			<< _wallpaper << info.wallpaperPath
			<< _index << info.index
			<< std::endl;
#endif
		return os;
...
#pragma endregion
	...
	std::vector<std::shared_ptr<InformationStructure>> Implement::allDesktopInfo()
	{
		return m_infoVector;
	}
	...
	bool Implement::switchDesktop(int index, bool withAnimation)
	{
		if ((m_infoVector.size() == 0) || (m_infoVector.size() > 0 && index > m_infoVector.size() - 1))
			return false;
		IVirtualDesktop* desktop = m_infoVector.at(index)->instance;
		HRESULT hr{};
		if (withAnimation)
			hr = m_virtualDesktopManagerInternal->SwitchDesktopWithAnimation(desktop);
		else
			hr = m_virtualDesktopManagerInternal->SwitchDesktop(desktop);
		if (SUCCEEDED(hr)) return true;
		return false;
	}
	...
	bool Implement::moveDesktop(int index)
	{
		if ((m_infoVector.size() == 0) || (m_infoVector.size() > 0 && index > m_infoVector.size() - 1))
			return false;
		CComPtr<IVirtualDesktop> currDesktop = nullptr;
		HRESULT hr = m_virtualDesktopManagerInternal->GetCurrentDesktop(&currDesktop);
		if (SUCCEEDED(hr))
		{
			hr = m_virtualDesktopManagerInternal->MoveDesktop(currDesktop, index);
			if (SUCCEEDED(hr))
			{
				update();
				return true;
	...
	bool Implement::switchDesktop(VirtualDesktopNS::API::IVirtualDesktop* desktop, bool withAnimation)
	{
		if ((m_infoVector.size() == 0) || !desktop)
			return false;
		HRESULT hr{};
	...
	bool Implement::pinActiveWindow()
	{
		return pinWindowByHwnd(GetForegroundWindow());
	}

	bool Implement::unpinActiveWindow()
	{
		return unpinWindowByHwnd(GetForegroundWindow());
	}

	bool Implement::ispinActiveWindow()
	{
		return ispinWindowByHwnd(GetForegroundWindow());
	}
	...
	LPWSTR Implement::_getApplicationId(HWND hwnd)
	{
		return LPWSTR();
	}
